...
const PlanMode = {
  get() { return localStorage.getItem("panda_mode") || "fast"; },
  set(mode) { localStorage.setItem("panda_mode", mode); },
};
...
/** Dedupe by normalized name; prefer local catalog (id) over OFF hits. */
function dedupeProducts(list) {
  ...
  for (const p of list) {
    ...
    if (p.id != null && prev.id == null) byName.set(key, p);
    else if (p.off_barcode && !prev.off_barcode) byName.set(key, p);
  }
